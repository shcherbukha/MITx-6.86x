...
def perceptron_single_step_update(
    ...
    completed.
    """
    agreement = label*(current_theta.dot(feature_vector) + current_theta_0)

    if abs(agreement) < 1e-5 or agreement < 0:
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            current_theta, current_theta_0 = perceptron_single_step_update(
                                                    ...
                                                    current_theta, 
                                                    current_theta_0)
    print ("perceptron: ", current_theta, current_theta_0)   
    return (current_theta, current_theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    current_theta = theta_sum = np.zeros(feature_matrix.shape[1])
    current_theta_0 = theta_0_sum = 0.0
    average_coeff = T*feature_matrix.shape[0]
    ...
    print ("average_perceptron: ", theta_sum/average_coeff, theta_0_sum/average_coeff)   
    return (theta_sum/average_coeff, theta_0_sum/average_coeff)
...
def pegasos(feature_matrix, labels, T, L):
    ...
    print ("pegasos: ", current_theta, current_theta_0)        
    return (current_theta, current_theta_0)
    raise NotImplementedError
...
def classify(feature_matrix, theta, theta_0):
    ...
    predictions[predictions < 0.0] = -1
    predictions[abs(predictions) < 1e-5] = -1
